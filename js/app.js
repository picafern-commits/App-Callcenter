...
function rotas(){
  ...
  const vehicles = [...new Set(all.map(r=>r.viatura).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt',{sensitivity:'base'}));
  const drivers = [...new Set(all.map(r=>r.condutor).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt',{sensitivity:'base'}));
  const totalKm = rows.reduce((sum,r)=>sum + routeKm(r), 0);
  const addCard = canEdit ? `<div class="card compact-form-card clean-side-card">
    <div class="card-head clean-card-head"><div><h3>Nova rota</h3><span class="muted">Registo rápido</span></div></div>
    <form id="routeForm" class="simple-stack-form route-form">
      <input class="field" name="viatura" placeholder="Viatura" required>
      <input class="field" name="matricula" placeholder="Matrícula">
      <input class="field" name="data" type="date" value="${today()}" required>
      <input class="field" name="pedidoPor" placeholder="Pedido por">
      <input class="field" name="destino" placeholder="Destino / serviço" required>
      ...
      <input class="field" name="condutor" placeholder="Condutor">
  ...
  return `<div class="grid ${canEdit ? 'two split-form-list clean-page-layout' : 'single-list'} routes-page clean-module-page">
    ${addCard}
...
function openRouteModal(id){
  ...
  openModal('Editar rota', `<form id="editRouteForm" class="form-grid">
    <input class="field" name="viatura" placeholder="Viatura" value="${esc(r.viatura||'')}" required>
    <input class="field" name="matricula" placeholder="Matrícula" value="${esc(r.matricula||'')}">
    <input class="field" name="data" type="date" value="${esc(r.data||today())}" required>
    <input class="field" name="pedidoPor" placeholder="Pedido por" value="${esc(r.pedidoPor||'')}">
    <input class="field" name="destino" placeholder="Destino / serviço" value="${esc(r.destino||'')}" required>
    ...
    <input class="field" name="condutor" placeholder="Condutor" value="${esc(r.condutor||'')}">
